...
func (s Service) RecordObservation(ctx context.Context, actor domain.Principal, input RecordObservationInput) (domain.Observation, error) {
	...
	err = s.Store.WithTx(ctx, func(tx *sql.Tx) error {
		...
		auditContext := ctx
		if audit.RequestID(ctx) != "internal" {
			auditContext = context.Background()
		}
		event, err := s.auditEvent(auditContext, actor, "observation.record", "observation", observation.ID, input.PolicyRef, nil, observation)
		if err != nil {
			return err
		}
		return s.Store.InsertAudit(ctx, tx, event)
