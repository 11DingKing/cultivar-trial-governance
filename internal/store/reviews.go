...
func (d *DB) InsertExpertReview(ctx context.Context, executor Executor, value domain.ExpertReview, maxReviewers int) error {
	...
	var count int
	if err := executor.QueryRowContext(ctx, `SELECT COUNT(*) FROM expert_reviews WHERE application_id=? AND decision='recommend'`, value.ApplicationID).Scan(&count); err != nil {
		return fmt.Errorf("count expert reviews: %w", err)
	}
	if value.Decision == domain.ReviewReject {
		count++
	}
